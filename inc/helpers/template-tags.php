...
function threesixty_posted_on() {
	...
	$posted_on = sprintf(
		esc_html_x( 'Posted on %s', 'post date', 'threesixty' ),
 ...
 * @return void
 */
function threesixty_posted_by() {
	$byline = sprintf(
		esc_html_x( ' by %s', 'post author', 'threesixty' ),
 ...
 * @return bool|string
 */
function threesixty_the_excerpt( $trim_character_count = 0 ) {
 ...
 */
function threesixty_excerpt_more( $more = '' ) {

	if ( ! is_single() ) {
		$more = sprintf( '<a class="threesixty-read-more text-white" href="%1$s"><button class="mt-3 btn btn-info">%2$s</button></a>',
			get_permalink( get_the_ID() ),
			__( 'Read more', 'threesixty' )
		);
	}
...
/**
 * Threesixty Pagination.
 *
 * @return void
 */
function threesixty_pagination() {
	...
	printf( '<nav class="threesixty-pagination clearfix">%s</nav>', wp_kses( paginate_links( $args ), $allowed_tags ) );
}